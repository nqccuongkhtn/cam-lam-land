...
mapAdsRouter.get('/active', async (_req, res, next) => {
  try {
    const rows = await query(
      `SELECT id, advertiser_name, advertiser_phone, image_url, points
    ...
    for (const r of rows) {
      ...
      for (const p of pts) {
        if (p && Number.isFinite(+p.lng) && Number.isFinite(+p.lat))
          ads.push({ id: r.id, name: r.advertiser_name, phone: r.advertiser_phone, image: r.image_url, lng: +p.lng, lat: +p.lat });
      }
    }
  ...
  try {
    const rows = await query(
      `SELECT id, advertiser_name AS "advertiserName", advertiser_phone AS "advertiserPhone",
              image_url AS "imageUrl", wards, points, package, status,
...
mapAdsRouter.post('/', adminRequired, async (req: AuthedRequest, res, next) => {
  try {
    ...
    const [row] = await query(
      `INSERT INTO map_ads (advertiser_name, advertiser_phone, image_url, wards, points, package, expires_at, status, created_by)
       VALUES ($1,$2,$3,$4::text[],$5::jsonb,$6, now() + make_interval(months => $7::int), 'active', $8)
       RETURNING id`,
      [name, phone, b.imageUrl ?? null, wards, JSON.stringify(points), b.package ?? `${months} tháng`, months, req.user!.id]);
