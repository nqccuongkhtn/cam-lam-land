...
async function geminiCall(model: string, buffer: Buffer, mime: string): Promise<string> {
  ...
  // TẮT bớt "thinking" cho phản hồi NHANH (đọc bảng số không cần suy luận, tránh timeout): Gemini 3 → thinkingLevel=MINIMAL; Gemini 2.5 → thinkingBudget=0. KHÔNG đặt cả hai (Gemini 3 sẽ lỗi).
  const thinkingConfig: any = /gemini-3/.test(model) ? { thinkingLevel: 'MINIMAL' } : { thinkingBudget: 0 };
  const body = {
    contents: [{ parts: [{ text: GEMINI_PROMPT }, { inline_data: { mime_type: mime || 'image/png', data: buffer.toString('base64') } }] }],
    generationConfig: { temperature: 0, maxOutputTokens: 4096, thinkingConfig },
  };
...
// Dùng model cấu hình (mặc định gemini-3-flash-preview). Nếu API từ chối tên model (4xx: sai tên/chưa có quyền) thì TỰ LÙI về gemini-2.5-flash để Gemini vẫn chạy, không rơi xuống OCR.space vì lỗi tên model.
