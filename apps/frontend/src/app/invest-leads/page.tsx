...
interface Item { id: number; name: string; phone: string; amount: string | null; tenure: string | null; note: string | null; status: string; createdAt: string; }
...
export default function InvestLeadsPage() {
  ...
  function reload() { api<{ items: Item[] }>('/invest').then((r) => setItems(r.items || [])).catch(() => {}); }
  useEffect(() => { if (user?.role === 'admin') reload(); }, [user]);

  async function setStatus(id: number, status: string) { try { await api(`/invest/${id}`, { method: 'PATCH', body: JSON.stringify({ status }) }); reload(); } catch (e: any) { alert(e.message); } }
  async function del(id: number) { if (!confirm('Xoá đăng ký này?')) return; try { await api(`/invest/${id}`, { method: 'DELETE' }); reload(); } catch (e: any) { alert(e.message); } }
    ...
    <div className="bg-slate-50 min-h-[calc(100vh-56px)] py-8">
      <div className="mx-auto max-w-5xl px-4">
        <div className="flex items-center justify-between mb-1">
          <h1 className="text-2xl font-extrabold text-[#0A2540]">Đăng ký góp vốn CamInvest ({items.length})</h1>
          <Link href="/admin" className="text-sm font-semibold text-slate-500 hover:text-[#0A2540]">← Quản trị</Link>
        </div>
        <p className="text-sm text-slate-500 mb-4">Khách để lại thông tin qua trang &quot;Góp vốn đầu tư&quot; (/dau-tu) — liên hệ tư vấn gói góp vốn.</p>
        ...
        {shown.length === 0 ? <div className="bg-white border border-dashed border-slate-300 rounded-2xl p-10 text-center text-slate-500">Chưa có đăng ký nào.</div> : (
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    <p className="text-sm text-slate-600 mt-1">
                      {i.amount && <>Dự kiến góp: <b>{i.amount}</b> · </>}
                      {i.tenure && <>Kỳ hạn: <b>{i.tenure}</b></>}
                    </p>
                    {i.note && <p className="text-sm text-slate-500 mt-1 whitespace-pre-line">{i.note}</p>}
