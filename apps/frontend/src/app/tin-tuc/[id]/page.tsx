...
interface Article { title: string; url: string; source?: string; image?: string; summary?: string; body?: string; publishedAt?: string }

export default function NewsReader() {
  const { id } = useParams<{ id: string }>();
  let url = '';
  try { url = id ? atob(decodeURIComponent(id).replace(/-/g, '+').replace(/_/g, '/')) : ''; } catch { url = ''; }
  const [a, setA] = useState<Article | null>(null);
  const [done, setDone] = useState(false);

  useEffect(() => {
    fetch('/feed/news').then((r) => r.json()).then((d) => { setA((d.news || []).find((x: any) => x.url === url) || null); }).catch(() => {}).finally(() => setDone(true));
  }, [url]);
  ...
  if (!a) return (
    <div className="mx-auto max-w-3xl px-4 py-16 text-center">
      ...
      <p className="text-slate-500 mt-1 text-sm">Tin chỉ được lưu trong 7 ngày.{url ? ' Bạn có thể đọc tại nguồn:' : ''}</p>
      {url && <a href={url} target="_blank" rel="noreferrer" className="mt-4 inline-block bg-[#0A2540] text-white font-bold px-5 py-2.5 rounded-xl">Đọc bài gốc →</a>}
