...
export default function MapView({ center, zoom, className, layers = [], markers = [], overlays = [], baseMap = 'street', labels = true, measureMode = 'off', focusPoint = null, highlight = null, initialBounds, adMarkers = [], adOpacity = 1, fitTo = null, onMapClick, onMeasure }: Props) {
  ...
  useEffect(() => {
    ...
    adRefs.current = adMarkers.map((a) => {
      ...
      const nm = esc(String(a.name || '').slice(0, 16)), tel = esc(String(a.phone || ''));
      const center = a.image
        ...
        : `<text x="48" y="49" text-anchor="middle" dominant-baseline="middle" font-size="22" font-weight="800" fill="#C8A14B" font-family="Inter,sans-serif">${esc(String(a.name || 'C').charAt(0)).toUpperCase()}</text>`;
      el.innerHTML = `<svg width="78" height="78" viewBox="0 0 96 96" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"><defs><clipPath id="c${uid}"><circle cx="48" cy="48" r="30"/></clipPath><path id="t${uid}" d="M 10 48 A 38 38 0 0 1 86 48"/><path id="b${uid}" d="M 86 48 A 38 38 0 0 1 10 48"/></defs><circle cx="48" cy="48" r="47" fill="#0A2540"/><circle cx="48" cy="48" r="45.5" fill="none" stroke="#C8A14B" stroke-width="0.9"/>${center}<circle cx="48" cy="48" r="31" fill="none" stroke="#C8A14B" stroke-width="1.3"/><text font-family="Inter,sans-serif" font-size="8.4" font-weight="800" letter-spacing="0.3" fill="#ffffff"><textPath xlink:href="#t${uid}" startOffset="50%" text-anchor="middle">${nm}</textPath></text><text font-family="Inter,sans-serif" font-size="8.4" font-weight="800" letter-spacing="0.8" fill="#ffffff"><textPath xlink:href="#b${uid}" startOffset="50%" text-anchor="middle">${tel}</textPath></text><text x="8.5" y="48" text-anchor="middle" dominant-baseline="central" font-size="7.5" fill="#C8A14B">★</text><text x="87.5" y="48" text-anchor="middle" dominant-baseline="central" font-size="7.5" fill="#C8A14B">★</text></svg>`;
